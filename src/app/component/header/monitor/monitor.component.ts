...
export class MonitorComponent implements OnInit {
  ...
  tabList: Tab[] = [
    { text: '현장 모니터링', data: '현장 모니터링' },
    { text: 'CCTV 모니터링', data: 'CCTV 모니터링' },
    // { text: '실시간 위치 모니터링',
    //   data:'실시간 위치 모니터링'
    // }
  ];
  ...
  main() {
    ...
    if (this.device.platform_type < 3) {
      switch (userData.user_type) {
        ...
        case 'COMPANY':
          if (
            userData.user_role === 'MASTER_HEAD' ||
            userData.user_role === 'MASTER_GENERAL'
          ) {
    ...
    const modal = await this.modal.create({
      ...
      enterAnimation: (baseEl: HTMLElement) => {
        ...
        const wrapperEl = <HTMLElement>(
          baseEl.getElementsByClassName('modal-wrapper')[0]
        );
  ...
  chageRegionPosition() {
    ...
    const res = new CustomEvent('change_hq_regional:get()', {
      detail: { hq_regional_id: this.form.hq_regional_id },
    });
    window.dispatchEvent(res);
  }
  changeDistrictPosition() {
    // console.log('changeProjectPosition - ', this.form.project_id);
    const res = new CustomEvent('change_district:get()', {
      detail: { district_id: this.form.district_id },
    });
    window.dispatchEvent(res);
  }
  ...
  changeProjectPosition() {
    console.log('changeProjectPosition - ', this.form.project_id);
    const res = new CustomEvent('change_project:get()', {
      detail: { project_id: this.form.project_id },
    });
    window.dispatchEvent(res);
  }

  changeContractorPosition() {
    console.log('changeContractorPosition - ', this.form.master_company_id);
    const res = new CustomEvent('change_contractor:get()', {
      detail: { master_company_id: this.form.master_company_id },
    });
    window.dispatchEvent(res);
  }
}
