...
@Component({
  selector: 'app-select-action-option',
  templateUrl: './select-action-option.component.html',
  styleUrls: ['./select-action-option.component.scss'],
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => SelectActionOptionComponent),
    multi: true
  }]
})
export class SelectActionOptionComponent implements OnInit, ControlValueAccessor {
  @HostListener('click') onClick() {
    this.el.nativeElement.querySelector('[name=select]').dispatchEvent(new Event('click'));
  }
  
  @Input() color:Color;
  @Input() label:string = "검색기준";
