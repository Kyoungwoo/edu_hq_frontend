...
export class ConfirmPendingListPage implements OnInit, OnDestroy {
   ...
   */
   async get(limit_no = this.form.limit_no) {
    ...
    this.res = await this.connect.run('/approval/board/req/get', this.form, { loading: true });
   ...
   */
   public async getMobile($event) {
    ...
    const res = await this.connect.run('/approval/board/req/get', this.form, { loading: true });
    if(res.rsCode === 0 ) {
      /**
