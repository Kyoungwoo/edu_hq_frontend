...
export class TodayDepartureStatusListPage implements OnInit {
  ...
  getForm() {
    ...
    this.form.hq_regional_id = this.listForm.hq_regional_id;
    this.form.district_id = this.listForm.district_id;
  ...
  async getSummary() {
    this.res = await this.connect.run('/work_state/current', this.form, {
      loading: true,
    });
  ...
  async getList(limit_no = this.listForm.limit_no) {
    ...
    this.res2 = await this.connect.run('/work_state/detail/list', this.form, {
      loading: true,
    });
  ...
  async detail(item: TodayDepartureStatusListItem, index) {
    if (this.detailList[index]) {
      ...
      const res = await this.connect.run('/work_state/detail/sub_list', form, {
        loading: true,
      });
      if (res.rsCode === 0) {
