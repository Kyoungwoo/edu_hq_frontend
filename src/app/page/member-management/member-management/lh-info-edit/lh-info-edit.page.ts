...
@Component({
  ...
  templateUrl: './lh-info-edit.page.html',
  styleUrls: ['./lh-info-edit.page.scss'],
})
export class LhInfoEditPage implements OnInit {
  ...
  public async overlapEmail() {
    ...
    const res = await this.connect.run('/usermanage/info/lh/overlap/email', { user_email, user_id });
  ...
  public async overlapPhone() {
    ...
    if (!user_id) return this.validator.user_id = null;
    const res = await this.connect.run('/usermanage/info/lh/overlap/phone', { user_phone, user_id });
  ...
  //기본정보
  async getItem() {
    const res = await this.connect.run('/usermanage/info/lh/basic/detail', this.form, {
  ...
  //소속정보
  async getBelong() {
    const res = await this.connect.run('/usermanage/info/lh/belong/detail', this.form, {
    });
  ...
  //기본정보 수정
  async BasicSubmit() {
    const res = await this.connect.run('/usermanage/info/lh/basic/update', {
      ...this.form,
      ...this.formBasic
    }, {});
    if (res.rsCode === 0) {
  ...
  //소속정보 수정
  async BelongSubmit() {
    const res = await this.connect.run('/usermanage/info/lh/belong/update', {
      ...this.form,
      ...this.formApproval
    }, {});
    if (res.rsCode === 0) {
