...
export class DangerAreaLogDetailInfo {
...
export class DangerAreaLogDetail_Theme {
...
@Component({
  selector: 'app-danger-area-log-detail',
  templateUrl: './danger-area-log-detail.page.html',
  styleUrls: ['./danger-area-log-detail.page.scss'],
})
export class DangerAreaLogDetailPage implements OnInit {
  @Input() machinery_id;
  @Input() list_data;

  updateStatus: boolean = false;

  form: DangerAreaLogDetailInfo = new DangerAreaLogDetailInfo();
  selectList = [];
      ...
      buttons: [
        { text: '아니오' },
        {
          ...
          handler: async () => {
            ...
            let json_obj = new DangerAreaLogDetail_Theme();
  ...
  async TransFileData() {
    ...
    await this.form.plan_file.forEach(async (item) => {
      await this.form.machinery_file.push(item);
    });
    await this.form.plan_json.insert.forEach(async (item) => {
      await this.form.machinery_json.insert.push(item);
    });
    await this.form.plan_json.delete.forEach(async (item) => {
      await this.form.machinery_json.delete.push(item);
    });

    await this.form.regist_file.forEach(async (item) => {
      await this.form.machinery_file.push(item);
    });
    await this.form.regist_json.insert.forEach(async (item) => {
      await this.form.machinery_json.insert.push(item);
    });
    await this.form.regist_json.delete.forEach(async (item) => {
      await this.form.machinery_json.delete.push(item);
    });

    await this.form.rental_file.forEach(async (item) => {
      await this.form.machinery_file.push(item);
    });
    await this.form.rental_json.insert.forEach(async (item) => {
      await this.form.machinery_json.insert.push(item);
    });
    await this.form.rental_json.delete.forEach(async (item) => {
      await this.form.machinery_json.delete.push(item);
    });

    await this.form.etc_file.forEach(async (item) => {
      await this.form.machinery_file.push(item);
    });
    await this.form.etc_json.insert.forEach(async (item) => {
      await this.form.machinery_json.insert.push(item);
    });
    await this.form.etc_json.delete.forEach(async (item) => {
      await this.form.machinery_json.delete.push(item);
    });

    this.form.machinery_json.insert.forEach((item, index) => {
    ...
    switch (type) {
      case 'save':
        if (
          ...
          this.user.userData.user_id == this.form.create_user_id
        )
          state = true;
        break;
      case 'delete':
